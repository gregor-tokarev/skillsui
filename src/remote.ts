...
export async function loadRemote(entry: TrackedEntry): Promise<LoadedRemote> {
  const input = sourceInput(entry);

  if (entry.sourceType === 'well-known') {
    const skills = await wellKnownProvider.fetchAllSkills(input);
  ...
  const root = await cloneRepo(parsed.url, ref);
  return {
    root,
